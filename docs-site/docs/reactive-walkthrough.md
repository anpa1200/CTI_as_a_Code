...
- [Step R1.5: Hands-On Evidence Analysis — VS Code Investigation](#step-r15-hands-on-evidence-analysis--vs-code-investigation)
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/00-cover.png" alt="LifeTech Pharma — CTI as a Code reactive investigation cover" />
</figure>
---
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/01-project-structure.png" alt="Project folder structure initialized from reactive template" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/02-folder-tree.png" alt="Folder tree — reactive investigation scaffold" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/03-project-yml.png" alt="project.yml filled with case metadata" />
</figure>
---
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/00b-scope-in-scope.png" alt="Scope document — in-scope systems" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/00d-pirs.png" alt="Priority Intelligence Requirements (PIRs)" />
</figure>
---
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/00f-evidence-tree.png" alt="VS Code Explorer — full evidence tree" />
</figure>
---
...
|---|---|---|
| Rainbow CSV | `mechatroner.rainbow-csv` | Column-aware CSV viewer with RBQL SQL query |
| REST Client | `humao.rest-client` | Run VirusTotal / Shodan API calls inline |
| Hex Editor | `ms-vscode.hexeditor` | Inspect binary files without leaving VS Code |
| Prettier | `esbenp.prettier-vscode` | Auto-format JSON/JSONL on open (`Shift+Alt+F`) |
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/04-vscode-extensions.png" alt="VS Code extensions installed for evidence analysis" />
</figure>
...
| `Ctrl+Shift+F` | Search across all open files (global pivot) |
| `Ctrl+Shift+O` | JSON Outline — navigate nested structure |
| `Shift+Alt+F` | Format current file (Prettier) |
| `F5` | Open RBQL query console (Rainbow CSV) |
| `Ctrl+G` | Go to line / offset (Hex Editor) |
...
| File | Format | What It Contains | GitHub |
|---|---|---|---|
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/06-crowdstrike-outline.png" alt="CrowdStrike alert JSON structure in VS Code" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/07-prevention-policy.png" alt="Prevention policy showing detect-only mode" />
</figure>
...
- **Hash (SHA256)** `3b4c14a87e5f9d8c2a1f4e6b9c0d2e7a1b3c5d8f2a4e6c8b0d3e5a7c1f4b8d2e` — `powershell.exe` behavior hash from alert
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/10-m365-rbql-results.png" alt="M365 message trace RBQL query results" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/11-cfo-phishing-discovery.png" alt="CFO phishing email discovery in message trace" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/12-azure-ad-signin-table.png" alt="Azure AD sign-in table — suspicious session from Istanbul" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/13-suspicious-signin.png" alt="Suspicious sign-in detail — token replay indicators" />
</figure>
...
| MFA satisfied | `null` | Token replay — MFA already satisfied when token was issued |
| Conditional access | `notApplied` | CA policy evaluation is skipped for replayed tokens |
| OS | `null` | Unknown, unregistered device |
| Time delta | +2h17m from sign-in 001 | Legitimate login 07:14; attacker replay 09:31 |
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/15-ngfw-rainbow-csv.png" alt="NGFW logs open in Rainbow CSV — column-aware view" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/16-exfil-flow-discovery.png" alt="381 MB exfiltration flow identified — 99% upload ratio" />
</figure>


**Column map** (RBQL uses positional names — `a1`…`a41` — not header names):

| RBQL | Header | RBQL | Header |
|---|---|---|---|
| `a1` | receive_time | `a22` | dport |
| `a5` | src | `a28` | bytes |
| `a6` | dst | `a29` | bytes_sent |
| `a9` | rule | `a30` | bytes_received |
| `a10` | srcuser | `a33` | elapsed |
| `a12` | app | `a34` | category |
| `a27` | action | `a41` | session_end_reason |

**In VS Code Explorer:** click `palo-alto/ngfw-flows.csv`. Press `F5` to open the RBQL console.

> **RBQL syntax in Rainbow CSV:** the file is implicit — **no `FROM` clause**. Type casting uses JavaScript: `parseInt()` not `CAST()`. String matching uses `.startsWith()` / `.includes()` not `LIKE`.
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/06b-beacon-pattern-results.png" alt="Beacon pattern query results — C2 traffic identified" />
</figure>
...
> RBQL does not allow `ORDER BY` in aggregate queries. With two external IPs the result is readable without sorting.
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/18-lateral-movement-flows.png" alt="Internal lateral movement flows — WMI/DCOM signature" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/17-beacon-timing.png" alt="Beacon timing analysis — 7-minute intervals across both hosts" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/19-c2-beacon-timeline.png" alt="C2 beacon timeline — activity across investigation period" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/20-dns-malware-timeline.png" alt="DNS queries mapped to malware activity timeline" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/21-per-host-beacon-count.png" alt="Per-host beacon count — two infected hosts confirmed" />
</figure>
...
> `ORDER BY` not allowed in aggregate queries — with two hosts the result is readable without sorting.
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/22-attacker-recon-lookup.png" alt="Attacker recon activity — external DNS queries before compromise" />
</figure>
...
- **Indicator** Attacker recon: `185.220.101.47` queried `vpn.lifetechpharma.com` 1 min before VPN login
...
| Search term | Jumps to |
|---|---|
| `xp_cmdshell` | Shell execution events |
| `AuditLog` | Adversary OPSEC recon (`SELECT`) and anti-forensics (`DELETE`) |
| `UploadFile` | The exfiltration command |
| `Compress-Archive` | The staging command |
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/23-dcsync-events.png" alt="DCSync events — EID 4662 krbtgt and Administrator from non-DC IP" />
</figure>
...
| Search term | Attack phase covered |
|---|---|
| `185.220.101.47` | Initial access: AiTM phishing, VPN infiltration, attacker recon |
| `telemetry-cdn-services.biz` | Persistence: C2 beaconing from both infected hosts |
| `svc_backup` | Lateral movement: SMB, xp_cmdshell chain, DCSync |
| `198.51.100.44` | Exfiltration: NGFW flow, DNS lookup, SQL upload command, EID 5156 |
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/24-pivot-exfil-ip.png" alt="Pivot from exfil IP — cross-file corroboration" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/25-pivot-svc-backup.png" alt="Pivot from svc_backup account — lateral movement confirmed" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/26-pivot-c2-domain.png" alt="Pivot from C2 domain — infrastructure mapping" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/27-pivot-attacker-ip.png" alt="Pivot from attacker VPN IP — full access timeline" />
</figure>


<figure>
<img src="/CTI_as_a_Code/img/lifetech/28-attack-chain-matrix.png" alt="Complete attack chain — AiTM to DCSync" />
</figure>
...
:::warning Training IPs — substitute for real enrichment practice
`203.0.113.87` and `198.51.100.44` are **RFC 5737 documentation addresses** (TEST-NET-2/3). They are never routed on the internet. VirusTotal, Shodan, and RIPEstat will return no meaningful data for them.

`185.220.101.47` is a real Tor exit node — it will return real VT/Shodan results and is safe to query.

**To practice real IOC enrichment**, substitute the documentation IPs with real C2 infrastructure from any public threat report, or use `185.220.101.47` as the only real IOC in this scenario.
:::
...
### NOTE: 203.0.113.87 and 198.51.100.44 are RFC 5737 documentation IPs.
###       Substitute with real IOCs for live enrichment results.
...
After clicking **Send Request**, the right pane shows the full JSON response. Use `Ctrl+F` to extract key fields:
...
**VT Passive DNS** (`Ctrl+F` → `host_name`): shows all domains that have resolved to `203.0.113.87` — pivot to find actor infrastructure beyond what appeared in this org's DNS logs.

**RIPEstat prefix-overview** (`Ctrl+F`):
- `asn` → ASN number of the hosting provider
- `holder` → org name (e.g. `"Hostwinds LLC"`)
- `country` → confirms hosting country

**RIPEstat DNS history** (`Ctrl+F` → `domain`): historical resolutions — shows whether the C2 IP was used under different domains before this campaign.

**ARIN RDAP** (`Ctrl+F`):
- `name` → IP block name
- `handle` → abuse contact handle — use this to file an abuse report
- `startAddress` / `endAddress` → full IP block; check if adjacent IPs appear in other cases

**RDAP domain** (`Ctrl+F`):
- `registrationStart` → registration date of C2/exfil domains
- `registrarName` → registrar used — cheap/anonymous registrars are a red flag

For the crt.sh response, `Ctrl+F` → `name_value` to see all co-hosted domains. `cdn-telemetry-update.biz` and `windows-cdn-service.net` appear — new IOCs not yet in the org's DNS logs. Switch to `dns-queries.csv` and `Ctrl+Shift+F` to check immediately.

**Commit the `.http` file — it is a reproducible audit trail of every enrichment query:**

```bash
git add 03-analysis/ioc-queries.http
git commit -m "PROJ-2024-001: IOC enrichment queries — VT, Shodan, crt.sh, RDAP, RIPEstat, ARIN"
```
...
|---|---|
| `destination_ip` | `91.211.251.245` — real C2 IP, port 443 |
| `url` | `https://91.211.251.245/ga.js` — Malleable C2 profile mimicking Google Analytics |
| `Cookie` | Base64-encoded beacon metadata in the HTTP Cookie header |
| `User-Agent` | `Mozilla/4.0 (compatible; MSIE 8.0...)` — hardcoded CS UA string |
| `ProxyServer` | Beacon installs proxy settings pointing to C2 |
| `long-sleeps` | VT tag — beacon sleeps between check-ins (configurable interval) |
...
- **Indicator** Minimal import table — dynamic API resolution; evades import-based static detection
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/s2-timeline-table.png" alt="Completed timeline — dual entry points, chronological reconstruction" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/s3-claims-ledger.png" alt="Claims ledger — structured assertions with evidence citations and PIR mapping" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/s4-attck-mapping.png" alt="ATT&CK mapping — 12 techniques, 7 detection gaps classified" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/s5-attribution-table.png" alt="Attribution assessment — confidence ladder, medium-high, Iranian-nexus cluster" />
</figure>
...
<figure>
<img src="/CTI_as_a_Code/img/lifetech/s7-git-history.png" alt="Git log — phase-by-phase investigation audit trail" />
</figure>
